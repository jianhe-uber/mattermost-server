	...
	t.Run("Returns default moderations with default roles", func(t *testing.T) {
		...
		for _, moderation := range moderations {
			...
			if moderation.Name == "manage_members" {
				...
				require.Equal(t, moderation.Roles["guests"]["value"], guestEnabled)
				require.Equal(t, moderation.Roles["guests"]["enabled"], guestEnabled)
			}

			require.Equal(t, moderation.Roles["members"]["value"], memberEnabled)
			require.Equal(t, moderation.Roles["members"]["enabled"], memberEnabled)
		}
	})
